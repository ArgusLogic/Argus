...
_DIR_BUDGET_SAFETY_MARGIN_S: float = 5.0  # budget 必须 ≤ tool_timeout - margin
_DIR_BUDGET_FLOOR_S: float = 5.0  # budget 最低地板（避免 tool_timeout 极小时 budget=0）


def _resolve_dir_budget() -> float:
    """计算 dir_bruteforce 的墙钟预算。

    budget = clamp(_DIR_WALL_BUDGET_S, floor, tool_timeout - safety_margin)

    若 config 不可读，退回 _DIR_WALL_BUDGET_S。这避免了 dir_bruteforce
    内部 budget 与 ToolRegistry.execute 的 timeout 同时到期导致 TOOL_TIMEOUT
    抢先于 wall_budget 早停结果（findings 20260508_074418 issue #1）。
    """
    try:
        from utils.config import get_section

        tool_timeout_raw = get_section("general").get("tool_timeout", 60)
        tool_timeout = float(tool_timeout_raw)
    except Exception:
        return _DIR_WALL_BUDGET_S

    upper_bound = tool_timeout - _DIR_BUDGET_SAFETY_MARGIN_S
    return max(_DIR_BUDGET_FLOOR_S, min(_DIR_WALL_BUDGET_S, upper_bound))
...
async def dir_bruteforce(url: str, concurrency: str = "10") -> str:
    ...
    if reason == "wall_budget":
        header_lines.append(
            f"⏱ 达到 {budget:.0f}s 墙钟预算，提前收尾；"
            f"如需完整枚举请提高 tool_timeout 或改用自定义字典"
        )
