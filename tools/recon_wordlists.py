"""内置侦察字典：子域名和常用目录路径。避免外部文件依赖。"""

SUBDOMAINS = [
    "www", "mail", "ftp", "smtp", "pop", "imap", "webmail", "mx",
    "ns1", "ns2", "ns3", "dns", "dns1", "dns2",
    "api", "api2", "api3", "rest", "graphql",
    "dev", "develop", "development", "staging", "stage", "stg",
    "test", "testing", "qa", "uat", "sandbox",
    "prod", "production", "live",
    "admin", "administrator", "panel", "manage", "management",
    "portal", "dashboard", "console",
    "app", "apps", "application", "mobile", "m",
    "web", "www2", "www3",
    "blog", "news", "media", "press", "wiki", "docs", "doc",
    "forum", "community", "support", "help", "helpdesk",
    "shop", "store", "pay", "payment", "checkout", "cart",
    "auth", "login", "sso", "oauth", "id", "identity", "account",
    "cdn", "static", "assets", "img", "images", "media", "files",
    "upload", "download", "dl",
    "db", "database", "mysql", "postgres", "redis", "mongo", "elastic",
    "git", "gitlab", "github", "bitbucket", "svn", "repo",
    "ci", "cd", "jenkins", "travis", "drone", "build",
    "monitor", "monitoring", "grafana", "prometheus", "nagios", "status",
    "log", "logs", "logging", "elk", "kibana",
    "vpn", "remote", "rdp", "ssh", "proxy", "gateway",
    "backup", "bak", "old", "legacy", "archive",
    "internal", "intra", "intranet", "corp", "office",
    "crm", "erp", "hr", "jira", "confluence",
    "chat", "im", "slack", "teams", "meet",
    "search", "solr", "elasticsearch",
    "cache", "memcache", "varnish",
    "queue", "mq", "rabbit", "kafka",
    "s3", "storage", "oss", "bucket",
    "demo", "example", "sample", "temp", "tmp",
    "beta", "alpha", "canary", "preview", "rc",
    "v1", "v2", "v3",
    "node", "node1", "node2", "server", "server1", "server2",
    "host", "host1", "host2", "cluster",
    "docker", "k8s", "kubernetes", "swarm",
    "aws", "azure", "gcp", "cloud",
    "secure", "ssl", "tls", "cert",
    "waf", "firewall", "security",
    "analytics", "stats", "track", "tracking", "pixel",
    "ad", "ads", "adserver",
    "email", "newsletter", "notify", "notification",
    "ws", "websocket", "socket", "realtime",
    "rpc", "grpc", "soap", "wsdl",
    "cname", "redirect", "link", "go", "short", "url",
]

DIRECTORIES = [
    "/", "/admin", "/admin/", "/administrator", "/login", "/wp-admin",
    "/wp-login.php", "/wp-content", "/wp-includes",
    "/.git", "/.git/config", "/.git/HEAD", "/.gitignore",
    "/.env", "/.env.local", "/.env.production", "/.env.backup",
    "/.svn", "/.svn/entries",
    "/.htaccess", "/.htpasswd",
    "/robots.txt", "/sitemap.xml", "/sitemap_index.xml",
    "/crossdomain.xml", "/security.txt", "/.well-known/security.txt",
    "/favicon.ico", "/humans.txt",
    "/api", "/api/", "/api/v1", "/api/v2", "/api/v3",
    "/api/docs", "/api/swagger", "/api/openapi",
    "/swagger", "/swagger-ui", "/swagger-ui.html", "/swagger.json",
    "/openapi.json", "/openapi.yaml",
    "/graphql", "/graphiql", "/playground",
    "/docs", "/doc", "/documentation", "/readme", "/README.md",
    "/changelog", "/CHANGELOG.md",
    "/health", "/healthz", "/ready", "/readyz", "/status", "/ping",
    "/metrics", "/prometheus",
    "/info", "/version", "/about",
    "/config", "/config.json", "/config.yaml", "/config.yml",
    "/settings", "/settings.json",
    "/backup", "/backup.sql", "/backup.zip", "/db.sql", "/dump.sql",
    "/database", "/phpmyadmin", "/adminer", "/pma",
    "/console", "/shell", "/terminal", "/cmd",
    "/debug", "/debug/", "/trace", "/profiler",
    "/test", "/test/", "/tests", "/testing",
    "/temp", "/tmp", "/cache",
    "/upload", "/uploads", "/files", "/media", "/assets",
    "/images", "/img", "/static", "/public", "/dist",
    "/css", "/js", "/fonts", "/vendor",
    "/node_modules", "/bower_components",
    "/package.json", "/package-lock.json", "/composer.json",
    "/Gemfile", "/requirements.txt", "/Pipfile",
    "/Makefile", "/Dockerfile", "/docker-compose.yml",
    "/.dockerignore", "/Vagrantfile",
    "/server-status", "/server-info",
    "/cgi-bin", "/cgi-bin/",
    "/xmlrpc.php", "/wp-json", "/wp-json/wp/v2/users",
    "/feed", "/rss", "/atom",
    "/register", "/signup", "/sign-up",
    "/logout", "/signout", "/sign-out",
    "/forgot-password", "/reset-password",
    "/profile", "/account", "/user", "/users",
    "/search", "/search/",
    "/download", "/downloads",
    "/billing", "/invoice", "/payment",
    "/error", "/404", "/500", "/403",
    "/log", "/logs", "/access.log", "/error.log",
    "/trace.axd", "/elmah.axd",
    "/.DS_Store", "/Thumbs.db",
    "/web.config", "/Web.config",
    "/crossdomain.xml", "/clientaccesspolicy.xml",
    "/license", "/LICENSE", "/LICENSE.txt",
    "/contributing", "/CONTRIBUTING.md",
    "/.editorconfig", "/.eslintrc", "/.prettierrc",
    "/Procfile", "/app.json", "/manifest.json",
    "/service-worker.js", "/sw.js",
    "/actuator", "/actuator/health", "/actuator/env", "/actuator/info",
    "/jolokia", "/jolokia/list",
    "/.well-known/openid-configuration",
    "/oauth/token", "/oauth/authorize",
    "/token", "/auth/token",
]
